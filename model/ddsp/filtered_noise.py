...
class FilteredNoise(nn.Module):
    def __init__(self, conf=default, attenuate_gain=1e-2):
        super(FilteredNoise, self).__init__()

        self.frame_length = conf.hop_length
        self.attenuate_gain = attenuate_gain
        self.filter_coeff_length = conf.n_noise_filters
        self.filter_window = nn.Parameter(torch.hann_window(self.filter_coeff_length * 2 - 1, dtype=torch.float32),
                                          requires_grad=False)
    ...
    def forward(self, z):
        ...
        ZERO_PHASE_FR_BANK = INPUT_FILTER_COEFFICIENT.unsqueeze(-1).expand(batch_num, frame_num, filter_coeff_length,
                                                                           2).contiguous()
        ...
        zero_paded_windowed_linear_phase_ir_bank = F.pad(windowed_linear_phase_ir_bank, (0, self.frame_length - 1))
        ...
        noise = torch.rand(batch_num, frame_num, self.frame_length, dtype=torch.float32).view(-1, self.frame_length).to(
            INPUT_FILTER_COEFFICIENT.device) * 2 - 1
        zero_paded_noise = F.pad(noise, (0, filter_coeff_length * 2 - 2))
        ...
        FILTERED_NOISE = torch.zeros_like(ZERO_PADED_NOISE).to(INPUT_FILTER_COEFFICIENT.device)
        ...
        output_signal = F.conv_transpose1d(filtered_noise.transpose(1, 2),
                                           overlap_add_filter,
